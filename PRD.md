...
- Browser-grounded: informational answers come from visible research in the user's local Google Chrome.
...
3. Jarvis routes the transcript as an action, local Chrome research task, screen-aware answer, or action followed by answer.
...
7. While that work continues, Jarvis can answer a screen question without replacing the active task; another browser or desktop action waits until the current task is stopped or completed.
...
- Visible local Google Chrome research for informational questions, including typed searches, result-page clicks, and scrolling before answering.
- Whole-phrase intent safeguards keep factual product names and information-seeking imperatives on local Chrome research instead of misclassifying substrings as Mac commands.
...
- Short screen answers have a separate task lifetime from active computer work; local Chrome research and other actions are refused until the current task finishes or is explicitly stopped.
...
- Factual questions and information-seeking imperatives still route to local Chrome research when the GPT router is unavailable or returns an incorrect action route.
...
- Asking an informational question during a long task does not cancel, replace, or erase that task; Jarvis explains that Chrome research can start after the active task finishes or is stopped.
