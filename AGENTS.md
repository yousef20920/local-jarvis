...
- **AI Chat / Agent Reasoning**: OpenAI GPT-5.5 through the Cloudflare Worker proxy using the Responses API. Informational questions use a required live `web_search` tool call and expose cited sources in the panel.
...
**Concurrent Secretary Interactions**: `CompanionManager` owns long-running computer work separately from the replaceable short-response task. Always Listen can route and answer a web or screen question without cancelling the computer task already in progress. Jarvis refuses a second action until the active task completes or the user explicitly says “Jarvis stop,” preventing two workflows from fighting over the same mouse and keyboard.

**Transient Failure Recovery**: Screen observation and cited web research retry up to four times, while retryable computer-use GPT requests retry up to eight times with capped exponential backoff. Only capture, research, and model inference are retried. Local tool actions remain outside the retry helper so uncertain clicks, submissions, keystrokes, and terminal commands are never automatically replayed.
...
| `CompanionManager.swift` | ~1449 | Central state machine. Owns continuous-listening maintenance, dictation, shortcut monitoring, screen capture, GPT-backed Jarvis voice routing, retrying source-backed web answers, saved-task voice controls, independent short-response and long-running task lifetimes, system speech responses, and overlay management. Tracks voice state, conversation history, cited internet sources, selected OpenAI model display, and cursor visibility. Transcripts route through web research, screen-aware answers, or the Jarvis computer-use agent loop. Vision answers can still escalate to a one-shot browser search via a `[SEARCH:query]` tag when needed. |
| `MenuBarPanelManager.swift` | ~243 | NSStatusItem + custom NSPanel lifecycle. Creates the menu bar icon, manages the floating companion panel (show/hide/position), installs click-outside-to-dismiss monitor. |
| `CompanionPanelView.swift` | ~1172 | SwiftUI panel content for the menu bar dropdown. Shows companion status, Always Listen controls, clickable internet sources, GPT-5.5 model status, Jarvis text command box with workflow progress, saved-task resume/discard controls, a persistent task-terminal-approval indicator, exact pending terminal commands with Allow once and Allow for task choices, permissions UI, DM feedback button, and quit button. Dark aesthetic using `DS` design system. |
...
| `JarvisAssistantManager.swift` | ~502 | Jarvis coordinator. Screen-independent commands run through the deterministic rule planner for instant execution; everything else runs through the computer-use agent loop. Rejects overlapping action workflows, persists and restores durable checkpoints, owns one-command and task-wide terminal confirmation plus resume/discard state, mirrors agent progress into observable workflow state, and returns explicit spoken results. |
...
| `JarvisOpenAIClient.swift` | ~366 | Worker-backed OpenAI Responses API client. Sends GPT-5.5 text, vision, and required live-web-search requests to the Cloudflare Worker without exposing the API key in the app, and extracts cited URLs from search responses. |
| `JarvisVoiceIntentRouter.swift` | ~339 | GPT-backed router classifying requests as action, screen-aware vision, internet-grounded research, or action-then-vision before dispatch. Deterministic corrections keep questions and information-seeking imperatives on the web-research path, and whole-phrase command matching prevents names such as OpenAI or words such as research from being mistaken for Mac commands. |
